...
// export function realTimeSubscribe(callback) {
//     const unsubscribe = client.subscribe(['documents'], response => {
//         const eventType = response.events[0];
//         const changedDocuments = response.payload;

//         callback(eventType, changedDocuments);
//     });
...
export const rejectFriendRequest = async (userId, friendId) => {
    try {
        ...
        // Remove userId from friendId's friendRequests
        const updatedFriendRequests = (friendUser.friendRequests || []).filter(id => id !== friendId);
        await databases.updateDocument(databaseId, userCollectionId, userId, {
            friendRequests: updatedFriendRequests,
        });
    } catch (error) {
...
export const getFriends = async (userId) => {
    try {
        ...
        return friendsWithDetails.map(friend => ({
            ...
            displayName: friend.displayName,    // Friend's username
