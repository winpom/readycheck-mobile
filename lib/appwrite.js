...
export const getRSVPsForReadyCheck = async (readycheckId) => {
    try {
        const response = await databases.listDocuments(
            databaseId,
            RSVPsCollectionId,
            [Query.equal("readycheckId", readycheckId)] // Query on direct field, not virtual relationship
        );
...
//         // Parse the existing RSVPs or initialize as an empty array
//         let rsvps = readycheck.rsvps ? JSON.parse(readycheck.rsvps) : [];

//         // Find existing RSVP for this user
//         const userIndex = rsvps.findIndex(rsvp => rsvp.userId === userId);

//         if (userIndex !== -1) {
//             // Update the existing RSVP
//             rsvps[userIndex].status = rsvpStatus;
//         } else {
//             // Add a new RSVP entry for the user
//             rsvps.push({ userId, status: rsvpStatus });
//         }

//         // Convert back to JSON string and update the document
//         await databases.updateDocument(databaseId, readycheckCollectionId, readycheckId, {
//             rsvps: JSON.stringify(rsvps),
//         });
//     } catch (error) {
