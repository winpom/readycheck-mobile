...
// Function to update RSVP status for a user in a readycheck
export const updateRSVP = async (readycheckId, userId, rsvpStatus) => {
    try {
        // Fetch the readycheck document
        const readycheck = await databases.getDocument(databaseId, readycheckCollectionId, readycheckId);

        // Parse existing RSVPs or initialize as an empty array if RSVPs is undefined or empty
        let RSVPs = [];
        if (readycheck.RSVPs && typeof readycheck.RSVPs === "string" && readycheck.RSVPs.trim() !== "") {
            RSVPs = JSON.parse(readycheck.RSVPs);
        }

        // Remove any existing RSVP for the user, then add the new RSVP
        const updatedRSVPs = RSVPs.filter(rsvp => rsvp.userId !== userId);
        updatedRSVPs.push({ userId, status: rsvpStatus });

        // Convert the updated RSVPs array back to JSON string
        const rsvpString = JSON.stringify(updatedRSVPs);

        // Update the document with the new RSVPs JSON string
