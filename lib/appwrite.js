...
export const createReadyCheck = async (form) => {
    try {
        ...
        const senderName = `${ownerDocument.displayName || ownerDocument.username}`
        ...
        for (const inviteeId of form.invitees) {
            ...
            await databases.createDocument(databaseId, notificationCollectionId, ID.unique(), {
                ...
                message: `${senderName} invited you to ${form.title}`,
            ...
            if (inviteeDocument.expoPushToken) {
                await sendPushNotification(
                    ...
                    `${senderName} invited you to join ${form.title} at ${formattedTimingString}`,
    ...
    try {
        ...
        for (const inviteeId of newInvitees) {
            ...
            const notificationData = {
                type: "readyCheckInvite",
                message: `${senderName} invited you to ${existingReadyCheck.title}`,
                userId: inviteeId,
                senderId: ownerDocument.$id,
                readycheckId: readycheckId,
                timestamp: new Date().toISOString(),
            ...
            if (inviteeDocument.expoPushToken) {
                await sendPushNotification(
                    inviteeDocument.expoPushToken,
                    'ReadyCheck Invitation',
                    `${senderName} invited you to join ${existingReadyCheck.title} at ${formattedTimingString}`,
...
export const addOrUpdateRSVP = async (readycheckId, userId, rsvpStatus) => {
    try {
        ...
        await databases.createDocument(
            ...
            {
                type: "readyCheckInvite",
                message: `${currentUserName} responded to ${readycheck.title} with "${rsvpStatus}"`,
                userId: ownerId,
        ...
        if (ownerDocument.expoPushToken) {
            await sendPushNotification(
                ...
                'RSVP Update',
                `${currentUserName} responded to ${readycheck.title} with "${rsvpStatus}".`,
...
export const requestFriend = async (userId, friendId) => {
    try {
        ...
        await databases.createDocument(
            ...
            {
                type: "friendInvite",
                message: `${currentUserName} sent a friend request.`,
                userId: friendId,
                senderId: userId,
                timestamp: new Date().toISOString(),
                readycheckId: null,
